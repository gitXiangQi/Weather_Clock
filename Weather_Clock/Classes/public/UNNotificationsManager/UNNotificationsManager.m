...
+ (void)registerWHHXLocalNotification API_AVAILABLE(ios(10.0)) {
    UNUserNotificationCenter *center = [UNUserNotificationCenter currentNotificationCenter];
    [center requestAuthorizationWithOptions:(UNAuthorizationOptionBadge | UNAuthorizationOptionSound | UNAuthorizationOptionAlert) completionHandler:^(BOOL granted, NSError * _Nullable error) {
        if (!error) {
            NSLog(@"request authorization succeeded!");
...
+ (void)logErrorWith:(NSError *)error {
}
...
+ (void)addComponentsNotificationWithWHHXBody:(NSString *)body title:(NSString *)title subTitle:(NSString *)subTitle dateComponents:(NSDateComponents *)components music:(NSString *)music identifer:(NSString *)identifer isRepeat:(BOOL)repeat completionHanler:(void (^)(NSError *))handler API_AVAILABLE(ios(10.0)) {
    [self logErrorWith:nil];
    UNNotificationContent *content = [self contentWithTitle:title subTitle:subTitle body:body sound:[UNNotificationSound soundNamed:music]];
...
+ (void)getAllNotificationWHHXIdentiferBlock:(void (^)(NSArray<NSString *> *identifers))idBlock API_AVAILABLE(ios(10.0)) {
    [[self center] getDeliveredNotificationsWithCompletionHandler:^(NSArray<UNNotification *> * _Nonnull notifications) {
        NSMutableArray *array= [NSMutableArray array];
        [self logErrorWith:nil];
        [notifications enumerateObjectsUsingBlock:^(UNNotification * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (UNMutableNotificationContent *)contentWithWHHXTitle:(NSString *)title subTitle:(NSString *)subTitle body:(NSString *)body {
    [self logErrorWith:nil];
    NSString *titleStr = title.length > 0 ? title : @"闹钟";
    NSString *bodyStr = body.length > 0 ? body : @"闹钟";
    UNMutableNotificationContent *content = [[UNMutableNotificationContent alloc] init];
...
+ (UNMutableNotificationContent *)contentWithWHHXTitle:(NSString *)title subTitle:(NSString *)subTitle body:(NSString *)body badge:(NSInteger)badge sound:(UNNotificationSound *)sound attachments:(NSArray<UNNotificationAttachment *> *)attachments {
    UNMutableNotificationContent *content = [self contentWithTitle:title subTitle:subTitle body:body badge:badge sound:sound];
    content.attachments = attachments;
    return content;
}
...
- (void)userNotificationWHHXCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions))completionHandler  API_AVAILABLE(ios(10.0)) {
    if([notification.request.trigger isKindOfClass:[UNPushNotificationTrigger class]]) {
        NSDictionary * userInfo = notification.request.content.userInfo;
        [[NSNotificationCenter defaultCenter] postNotificationName:UNDidReciveRemoteNotifationKey object:nil userInfo:userInfo];
    }else {
